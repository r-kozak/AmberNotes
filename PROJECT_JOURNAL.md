...
**Поточна версія:** v0.3: The Vault (SQLCipher):
...
- [ ] Крок 13: Екран авторизації (App Lock): Реалізація `LoginView.axaml` — повноекранний overlay в "бурштинових" тонах (#2B1B10, #FFBF00), поля пароля + підтвердження (перший запуск), кнопка входу, індикатор завантаження.
- [ ] Крок 14: Логіка ініціалізації: перевірка перший запуск/відкриття існуючого сховища. Оновлення журналу та фіксація архітектури безпеки.
...
- **Наступний крок:** Кроки 13-14 — `LoginView.axaml` (UI overlay у бурштинових тонах) + логіка ініціалізації.
